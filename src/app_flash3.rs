...
pub fn flash_raft_app(
    ...
    #[cfg(unix)]
    let usb_info = if let Some(usb_port) = serial_port.as_any().downcast_ref::<TTYPort>() {
        usb_port.port_info().usb_info.clone()
    } else {
        None
    };

    #[cfg(windows)]
    let usb_info = if let Some(usb_port) = serial_port.as_any().downcast_ref::<COMPort>() {
        usb_port.port_info().usb_info.clone()
    } else {
        None
    };
